    ...
    try {
      ...
      if (cityUse.length == 0) {
        try {
          ...
          if (cityExistente.length != 0) {
            try {
              const data = await sequelize.query("DELETE FROM cities WHERE id_city=?", {
                replacements: [idCity],
                type: sequelize.QueryTypes.DELETE,
              });
              res.status(200).json({ msj: "Ciudad eliminada exitosamente" });
            } catch (err) {
              console.log("error" + err);
            }
          } else {
    ...
    try {
      ...
      if (countryUse.length == 0) {
        try {
          ...
          if (countryExistente.length != 0) {
            try {
              const data = await sequelize.query("DELETE FROM countries WHERE id_country=?", {
                   replacements: [idCountry],
                   type: sequelize.QueryTypes.DELETE,
              });
              res.status(200).json({ msj: "Pais eliminado exitosamente" });
            } catch (err) {
              console.log("error" + err);
            }
          } else {
    ...
    try {
      ...
      if (regionUse.length == 0) {
        try {
          ...
          if (regionExistente.length != 0) {
            try {
              const data = await sequelize.query("DELETE FROM regions WHERE id_region=?", {
                   replacements: [idRegion],
                   type: sequelize.QueryTypes.DELETE,
              });
              res.status(200).json({ msj: "Region eliminada exitosamente" });
            } catch (err) {
              console.log("error" + err);
            }
          } else {
