...
module.exports = {
      ...
      if (contactExistente.length == 0) {
        try {
          const data = await sequelize.query(
            "INSERT INTO contacts (id_contact,fname,lname,position,email,id_company,id_city,address,interest,account_phone,preference_phone,account_whatsapp,preference_whatsapp,account_instagram,preference_instagram,account_facebook,preference_facebook,account_linkedin,preference_linkedin,active) VALUES (NULL,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            {
              replacements: [
                fname,
                lname,
  ...
  deleteContact: async (req, res) => {
    ...
    try {
      ...
      console.log('contactExistente')
      if (contactExistente.length != 0) {
        try {
            const data = await sequelize.query("DELETE FROM contacts WHERE id_contact=?", {  
            replacements: [idContact],
            type: sequelize.QueryTypes.DELETE,
          });
          res.status(200).json({ msj: "Contacto eliminado exitosamente" });
        } catch (err) {
          console.log("error" + err);
        }
      } else {
